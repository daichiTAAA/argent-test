...
class RelayManager {
  ...
  async relay(
    ...
    const methodData = _module.contract.methods[_method](
      ..._params
    ).encodeABI();

    const gasLimit = await RelayManager.getGasLimitRefund(
      _module,
      _method,
      _params,
      _wallet,
    ...
    if (_refundToken !== ETH_TOKEN) {
      gasPrice = (
        await (
          await TestSimpleOracle.at(this.priceOracle)
        ).ethToToken(_refundToken, _gasPrice)
      ).toNumber();
    }
    ...
    const executeData = _module.contract.methods
      .execute(
        _wallet.address,
        methodData,
        nonce,
    ...
    const nonZerosString = executeData
      .toString()
      .slice(2)
      .replace(/00(?=(..)*$)/g, "");
  ...
  */
  static async getGasLimitRefund(
    _module,
    _method,
    _params,
    _wallet,
    ...
    try {
      gasEstimateFeatureCall = await _module.contract.methods[_method](
        ..._params
      ).estimateGas({ from: _module.address });
    ...
    const requiredSignatures = await _module.getRequiredSignatures(
      _wallet.address,
      methodData
    );
    ...
    const gasLimit =
      ...
      requiredSigsGas +
      nonceCheckGas +
      10000 * _signers.length +
      gasEstimateFeatureCall +
      refundGas;
  ...
  async debugGasLimits(_module, _method, _params, _wallet, _signers) {
    ...
    requiredSigsGas = await _module.getRequiredSignatures.estimateGas(
      _wallet.address,
      methodData
    );
    ...
    for (let index = 0; index < _signers.length; index += 1) {
      ...
      const isGuardian = await (
        await IGuardianStorage.at(this.guardianStorage)
      ).isGuardian(_wallet.address, signer);
