...
const fundTokens = async (tokenHolder, infrastructure) => {
  ...
  const USDC = await IUSDCToken.at(
    "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"
  );
  const USDT = await TetherToken.at(
  ...
  if (!tokensFunded) {
    ...
    await WETH.transfer(tokenHolder, web3.utils.toWei("10000"), {
      from: "0x2F0b23f53734252Bda2277357e97e1517d6B042A",
    });
    await stETH.transfer(tokenHolder, web3.utils.toWei("100"), {
      from: "0xa2f987a546d4cd1c607ee8141276876c26b72bdf",
    });
    await DAI.transfer(tokenHolder, web3.utils.toWei("1000000"), {
      from: "0x5d3a536E4D6DbD6114cc1Ead35777bAB948E3643",
    });
    await sUSD.transfer(tokenHolder, web3.utils.toWei("1000000"), {
      from: "0x6c5024cd4f8a59110119c56f8933403a539555eb",
    });

    // mint USDC
    await USDC.configureMinter(infrastructure, web3.utils.toWei("10000000"), {
      from: "0xe982615d461dd5cd06575bbea87624fda4e3de17",
    });
    await USDC.mint(tokenHolder, web3.utils.toWei("10000000"));

    // mint USDT
    await USDT.issue(web3.utils.toWei("1000000"), {
      from: "0xc6cde7c39eb2f0f0095f41570af89efc2c1ea828",
    });
    await USDT.transfer(tokenHolder, web3.utils.toWei("1000000"), {
      from: "0xc6cde7c39eb2f0f0095f41570af89efc2c1ea828",
    });
...
module.exports.deployArgent = async (accountsArray, options = {}) => {
  ...
  await moduleRegistry.registerModule(
    module.address,
    ethers.utils.formatBytes32String("ArgentModule")
  );
  ...
  const createFundedWallet = async (amounts = {}) => {
    ...
    for (const [ticker, amount] of Object.entries(amounts)) {
      if (ticker !== "ETH") {
        ...
        await token.transfer(walletAddress, amount.toString(), {
          from: tokenHolder,
        });
      }
    }
  ...
  async function tokenBalances(address) {
    ...
    return {
      ...
      USDC: new BN(await tokens.USDC.balanceOf(account))
        .div(new BN(1e6))
        .toString(),
