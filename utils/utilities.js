...
const utilities = {
  ...
  signOffchain: async (
    ...
    const messageHash = utilities.getMessageHash(
      from,
      value,
      data,
      chainId,
  ...
  personalSign: async (signHash, signer) =>
    ethers.utils.joinSignature(signer.signingKey.signDigest(signHash)),
  ...
  decodeLogs: (logs, emitter, eventName) => {
    ...
    return logs
      ...
      .map((log) =>
        web3.eth.abi.decodeLog(eventABI.inputs, log.data, log.topics.slice(1))
      )
  ...
  versionFingerprint: (modules) => {
    const concat = modules
      ...
      .reduce(
        (prevValue, currentValue) => prevValue + currentValue.slice(2),
        "0x"
      );
  ...
  increaseTime: async (seconds) => {
    ...
    if (!client.includes("TestRPC")) {
      console.warn("Client is not ganache-cli and cannot forward time");
    } else {
  ...
  initNonce: async (wallet, module, manager, securityPeriod) => {
    const nonceInitialiser = (
      await utilities.getNamedAccounts()
    ).freeAccounts.slice(-1)[0];
    await utilities.addTrustedContact(
      wallet,
      nonceInitialiser,
      module,
      securityPeriod
    ...
    const owner = await wallet.owner();
    const transaction = utilities.encodeTransaction(
      nonceInitialiser,
      1,
      ZERO_BYTES
    );
  ...
  createWallet: async (factoryAddress, owner, modules, guardian) => {
    ...
    const event = await utilities.getEvent(
      tx.receipt,
      factory,
      "WalletCreated"
    );
  ...
  swapAndCheckBalances: async ({ swap, bought, sold, wallet }) => {
    const balanceOf = (token) =>
      (token === ETH_TOKEN ? utilities.getBalance : token.balanceOf)(
        wallet.address
      );
    const getBalances = async () => [
      await balanceOf(sold),
      await balanceOf(bought),
    ];
